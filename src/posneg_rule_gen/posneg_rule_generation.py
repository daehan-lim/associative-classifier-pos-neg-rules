    ...
    for c in classes:
        if not c.issubset(itemset):
            ...
            if r > corr:  # generate positive rule
                ...
                if confidence(combined_support_count, class_support_count_dict[c]) >= min_conf:
                    PCR.append({'antecedent': itemset, 'consequent': c_str})
                ...
                if confidence(combined_support_count_nr1, class_support_count_dict[c]) >= min_conf:
                    NCR.append({'antecedent': neg_itemset, 'consequent': c_str})
                if confidence(combined_support_count_nr2, class_support_count_dict[c]) >= min_conf:
                    NCR.append({'antecedent': itemset, 'consequent': '!' + c_str})
