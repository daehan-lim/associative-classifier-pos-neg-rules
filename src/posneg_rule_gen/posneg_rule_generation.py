...
def ponerg(itemset, classes, class_support_count_dict, corr, min_conf, transactions_df):
    ...
    for c in classes:
        ...
        r = correlation(itemset, c, transactions_df, antecedent_support_count, combined_support_count, class_support_count_dict[c])
        c_str, = c
        if r > corr:  # generate positive rule
            ...
            not_i_support_count = util.get_item_support_count_df(itemset, transactions_df, negated=True)
            if (conf := confidence(combined_support_count_nr1, not_i_support_count)) >= min_conf:
            ...
            if (conf := confidence(combined_support_count_nr2, antecedent_support_count)) >= min_conf:
